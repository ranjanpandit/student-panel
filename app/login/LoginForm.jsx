    ...
    <div className="relative min-h-screen overflow-hidden px-4 py-10 sm:px-6 lg:px-10">
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="relative overflow-hidden rounded-[36px] bg-slate-950 px-7 py-10 text-white shadow-[0_40px_100px_rgba(15,23,42,0.25)] sm:px-10 lg:px-12">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(125,211,252,0.28),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(16,185,129,0.22),_transparent_22%)]" />
          <div className="relative flex h-full flex-col justify-between">
            <div className="max-w-xl">
              ...
              <h1 className="mt-8 font-display text-5xl leading-none tracking-[-0.05em] text-white sm:text-6xl">
                Clearer, sharper, and easier to use.
              </h1>
              <p className="mt-6 max-w-lg text-base leading-8 text-slate-200 sm:text-lg">
                Review your schedule, enter secure assessments, and track performance from a student portal with stronger contrast and better readability.
              </p>
            </div>

            <div className="grid gap-4 pt-10 sm:grid-cols-3">
              <FeatureCard title="Secure" copy="Protected exam flow with guided instructions and focused layouts." />
              <FeatureCard title="Organized" copy="A cleaner dashboard for exams, results, progress, and profile data." />
              <FeatureCard title="Readable" copy="Higher contrast text, stronger surfaces, and cleaner visual hierarchy." />
            </div>
          </div>
        </section>
        ...
        <section className="glass-panel flex items-center px-5 py-6 sm:px-8">
          <div className="mx-auto w-full max-w-md">
            ...
            <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-6 text-slate-700">
...
function Field({ label, type = "text", ...props }) {
  return (
    <label className="block">
      ...
      <input
        type={type}
        className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-950 shadow-sm outline-none transition placeholder:text-slate-500 focus:border-sky-500 focus:ring-4 focus:ring-sky-100"
        {...props}
      />
    </label>
  );
